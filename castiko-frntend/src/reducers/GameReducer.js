...
function trackStatus(action) {
  userInfo.push(action); console.log("push", userInfo);
  return userInfo;
}
...
export default function game(
    ...
    switch (action.type) {
    case ActionTypes.MOVE_CARD:console.log("movve card", action, state);
        trackStatus(action);
        return moveCards(state, action);
    case ActionTypes.TURN_CARD:console.log("turn card", action);
        return turnCard(state, action);
    case 'LOGOUT':
        postStatus();
        return "user logged out";
    case ActionTypes.RESUME_CARD:
        return moveCards(state, action);
